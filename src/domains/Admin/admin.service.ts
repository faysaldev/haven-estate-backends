...
const getRecentActivity = async (): Promise<RecentActivity[]> => {
  ...
  const [recentScheduleViews, recentRequestInfo, recentProperties] = await Promise.all([
  ...
  scheduleViewActivities.forEach((view: any) => {
    allActivities.push({
      ...
      details: `Scheduled view for property: ${view.property_id?.title || 'N/A'}`
    });
  });
...
const getTopPropertiesViews = async (): Promise<TopPropertyView[]> => {
  // For now, we'll simulate the top properties by counting related schedule views and request info
  // In a real application, you might have an actual view counter field in the property model
